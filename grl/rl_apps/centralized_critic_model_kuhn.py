...
class TorchCentralizedCriticModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config,
        ...
        self.model = get_valid_action_fcn_class(self.obs_dim, self.action_dim)(obs_space, action_space, num_outputs,
                                                       model_config,
                                                       name)  ### TODO: check why the value_function of this model is called! It should not be since we want to use only the central_value_function
        ### can figure this out by renaming this and never instantiate self. model
        # Central VF maps (obs, opp_obs, opp_act) -> vf_pred
        input_size = (self.obs_dim + self.action_dim)*2 + self.action_dim
