...
def centralized_critic_postprocessing_4p(policy,
                                      sample_batch,
    ...
    if (pytorch and hasattr(policy, "compute_central_vf")) or \
        ...
        opponent_batch = other_agent_batches[partner_id][1]
...
CCPPOTorchPolicy_4P = PPOTorchPolicy.with_updates(
    name="CCPPOTorchPolicy",
    postprocess_fn=centralized_critic_postprocessing_4p,
...
def get_policy_class(config):
    if config["framework"] == "torch":
        return CCPPOTorchPolicy_4P


CCTrainer_4P = PPOTrainer.with_updates(
    name="CCPPOTrainer",
...
if __name__ == "__main__":
    ...
    results = tune.run(CCTrainer_4P, config=config, stop=stop, verbose=1)
