...
from grl.rl_apps.centralized_critic_model_full_obs_larger_network_tiny_bridge_4p import \
    TorchCentralizedCriticModelFullObsLargerModelTinyBridge
from grl.rl_apps.centralized_critic_model_full_obs_larger_network_bridge_4p import \
    TorchCentralizedCriticModelFullObsLargerModelBridge
...
def get_all_psro_specs_with_prob(ckpt, psro_seed_path):
    payoff_table_json_path = os.path.join(psro_seed_path, "payoff_table_checkpoints",
                                          f"payoff_table_checkpoint_{ckpt}.json")
...
def get_all_sp_specs(sp_iter, sp_seed_path):
    all_specs = []
    player_0_br_iter_json_path = os.path.join(sp_seed_path, "br_policy_checkpoint_specs",
                                              f"best_response_player_0_iter_{sp_iter}.json")  # use 0 since it's the same for player 0 and player 1
    player_0_sp_spec = StrategySpec.from_json_file(player_0_br_iter_json_path)
    player_1_br_iter_json_path = os.path.join(sp_seed_path, "br_policy_checkpoint_specs",
                                              f"best_response_player_1_iter_{sp_iter}.json")  # use 0 since it's the same for player 0 and player 1
...
if __name__ == "__main__":
    ...
    if scenario_name == 'tiny_bridge_4p_psro_larger_model':
        # psro_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/tiny_bridge_4p_psro_larger_model/manager_12.20.01PM_Sep-23-2022/"
        psro_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/tiny_bridge_4p_psro_larger_model/manager_05.39.03PM_Oct-04-2022"
        sp_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/tiny_bridge_4p_psro_larger_model/tiny_bridge_4p_psro_larger_model_sparse_12.22.58PM_Sep-23-2022cddjb2ae/"
    elif scenario_name == 'bridge_bidding_psro_larger_model':
        # psro_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/bridge_bidding_psro_larger_model/manager_09.53.26PM_Sep-23-2022/"
        psro_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/bridge_bidding_psro_larger_model/manager_05.42.18PM_Oct-04-2022/"
        sp_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/bridge_bidding_psro_larger_model/bridge_bidding_psro_larger_model_sparse_09.54.34PM_Sep-23-2022t8hthd1c/"
    elif scenario_name == 'full_bridge_psro_larger_model':
        psro_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/full_bridge_psro_larger_model/manager_12.35.51PM_Sep-23-2022/"
        sp_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/full_bridge_psro_larger_model/full_bridge_psro_larger_model_sparse_12.38.48PM_Sep-23-2022jaxpr6ar"
    elif scenario_name == 'kuhn_4p_psro_full_obs_larger':
        psro_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/kuhn_4p_psro_full_obs_larger/manager_04.11.38PM_Sep-23-2022/"
        # psro_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/kuhn_4p_psro_full_obs_larger/manager_04.10.45PM_Sep-23-2022/"
        sp_path = "/marvel/home/smcaleer/dark_chess/nxdo/grl/data/kuhn_4p_psro_full_obs_larger/kuhn_4p_psro_full_obs_larger_sparse_10.12.26PM_Sep-23-20221zq15d5t"
    else:
        raise NotImplementedError

    output_dir = f'./head_to_head_results/head_to_head_{scenario_name}_n{num_games}'
    if args.random_policy:
        output_dir += '_rand'
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir)

    odd_ckpts = list(np.array(range(0, 500, 2)) + 1)  # Note: should > number of ckpts we have!
    ...
    psro_seeds_paths = [psro_path]
    sp_seeds_paths = [sp_path]
    ...
    for psro_seed_path in psro_seeds_paths:
        ...
        for ckpt in odd_ckpts:
            ...
            if os.path.isfile(payoff_table_json_path):
                payoff_table = PayoffTable.from_json_file(payoff_table_json_path)
                cur_timesteps = sum(
                    spec.metadata["timesteps_training_br"] for spec in payoff_table.get_ordered_spec_list_for_player(0))
                cur_timesteps += sum(
                    spec.metadata["timesteps_training_br"] for spec in payoff_table.get_ordered_spec_list_for_player(1))
    ...
    for sp_seed_path in sp_seeds_paths:
        ...
        for sp_iter in sp_iters:
            br_iter_json_path = os.path.join(sp_seed_path, "br_policy_checkpoint_specs",
                                             f"best_response_player_0_iter_{sp_iter}.json")  # use 0 since it's the same for player 0 and player 1
    ...
    for psro_seed_path, sp_seed_path in product(psro_seeds_paths, sp_seeds_paths):
        ...
        ckpt_mapping = {177: 500}
        print("\n\n\n\n")
        # ckpt_mapping = {17: 500}
        print(ckpt_mapping, "ckpt mapping\n\n\n\n")
        ...
        for ckpt, sp_iter in ckpt_mapping.items():
            ...
            for psro_team, sp_team in [[0, 1], [1, 0]]:
                ...
                for psro_spec_with_prob, sp_spec in product(psro_specs_with_prob, sp_specs):  # loop x * 1 times
                    if psro_spec_with_prob['weights'][psro_team] < 0.1:
                        continue
                    ...
                    for i in range(num_games):
                        ...
                        load_metanash_pure_strat(policies_for_each_player[psro_team],
                                                 pure_strat_spec=psro_spec_with_prob['specs'][psro_team])
                        if not args.random_policy:
                            load_metanash_pure_strat(policies_for_each_player[sp_team],
                                                     pure_strat_spec=sp_spec['specs'][sp_team])
                        ...
                        weighted_psro_rewards.append(
                            payoffs_per_team_this_episode[psro_team] * psro_spec_with_prob['weights'][psro_team])
                        psro_rewards.append(payoffs_per_team_this_episode[psro_team])
                    # print(sum(psro_rewards)/len(psro_rewards), psro_spec_with_prob['weights'][psro_team], 'psro payoffs, weights')
            ...
            print(np.mean(player_combo_results), "PSRO rewards")
        for psro_spec_with_prob in psro_specs_with_prob:
            print(psro_spec_with_prob["weights"])
        print("\n\n\n\n\n")
        print(ckpt_results, "ckpt results\n\n")
        print(ckpt_mapping, "ckpt mapping\n\n\n\n")
        # assert False
